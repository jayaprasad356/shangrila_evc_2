...
class BillController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $bill = Bill::where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->orWhere('bills.date', 'like', "%{$value}%")
                        ->orWhere('bills.id', 'like', "%{$value}%")
                        ->orWhere('bills.total', 'like', "%{$value}%")
        ...
        $bills = $bill->join('users', 'bills.user_id', '=','users.id')
        ->select('bills.id AS id','bills.*','users.email AS email')
        ->latest()->paginate(Helpers::getPagination())->appends($query_param);
        return view('admin-views.bills.list', compact('bills', 'search'));
    }
